...
class BERT_BiLSTM_Model(nn.Module):
    def __init__(self, hidden_dim, num_classes, bert_model_name='bert-base-uncased'):
        """
        :param hidden_dim: Hidden dimension size for BiLSTM
        ...
        # BiLSTM 层，设置 bidirectional=True
        self.lstm = nn.LSTM(input_size=768, hidden_size=hidden_dim // 2,  # hidden_dim // 2 因为 BiLSTM 的输出拼接
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # BiLSTM 输出 shape: (batch_size, sequence_length, hidden_dim)
        # 获取 LSTM 的最后一个时间步的输出
        lstm_last_hidden = lstm_out[:, -1, :]  # 拼接的双向隐藏状态
